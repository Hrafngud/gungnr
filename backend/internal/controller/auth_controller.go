...
func (c *AuthController) Login(ctx *gin.Context) {
	...
	if err != nil {
		respond.Err(ctx, errs.New(errs.CodeAuthStateGenerate, "failed to generate state"), errs.CodeAuthStateGenerate, "failed to generate state")
		return
	}
...
func (c *AuthController) Callback(ctx *gin.Context) {
	...
	code := ctx.Query("code")
	if state == "" || code == "" {
		respond.Err(ctx, errs.New(errs.CodeAuthCallbackMissing, "missing code or state"), errs.CodeAuthCallbackMissing, "missing code or state")
		return
	}

	cookieState, err := ctx.Cookie(oauthStateCookie)
	if err != nil || cookieState != state {
		respond.Err(ctx, errs.New(errs.CodeAuthStateInvalid, "invalid oauth state"), errs.CodeAuthStateInvalid, "invalid oauth state")
		return
	}
	...
	if err != nil {
		respond.Err(ctx, err, errs.CodeAuthLoginFailed, "login failed")
		return
	}
	...
	if err != nil {
		respond.Err(ctx, errs.New(errs.CodeAuthSessionCreate, "failed to create session"), errs.CodeAuthSessionCreate, "failed to create session")
		return
	}
...
func (c *AuthController) Me(ctx *gin.Context) {
	...
	if err != nil {
		respond.Err(ctx, err, errs.CodeAuthUnauthenticated, "unauthenticated")
		return
	}

	respond.OK(ctx, models.AuthUserResponse{
		ID:        session.UserID,
		Login:     session.Login,
...
func (c *AuthController) TestToken(ctx *gin.Context) {
	var req models.TestTokenRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		respond.Err(ctx, errs.New(errs.CodeAuthTestTokenInvalid, "invalid payload"), errs.CodeAuthTestTokenInvalid, "invalid payload")
		return
	}

	user, err := c.service.AuthenticateAdmin(ctx.Request.Context(), req.Login, req.Password)
	if err != nil {
		respond.Err(ctx, err, errs.CodeAuthLoginFailed, "failed to issue token")
		return
	}
	...
	respond.OK(ctx, models.TestTokenResponse{
		Token:     value,
		TokenType: "Bearer",
		ExpiresAt: session.ExpiresAt,
