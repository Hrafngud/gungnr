...
let spear: SVGElement | null = null
let spearRunning = false
let reduceMotion = false

const triggerSpearPulse = () => {
  if (reduceMotion || spearRunning || !root.value) return
  spearRunning = true
  root.value.classList.remove('spear-pulse')
  void root.value.offsetWidth
  root.value.classList.add('spear-pulse')
}
...
onMounted(() => {
  ...
  spear = root.value?.querySelector('.hero-spear') ?? null

  if (!spear) return

  spear.addEventListener('animationend', handleAnimationEnd)
  document.addEventListener('mousemove', handleMouseMove)
})

onBeforeUnmount(() => {
  if (spear) {
    spear.removeEventListener('animationend', handleAnimationEnd)
  }
...
<template>
  <div ref="root" class="hero-logo-mark" aria-hidden="true">
    <svg viewBox="124 82 252 338" class="hero-logo-svg" focusable="false">
      <path
        class="hero-path hero-path-1 hero-spear"
        d="M263.00,342.00 L235.00,343.00 L229.00,352.00 L248.00,417.00 L253.00,414.00 L268.00,361.00 L269.00,352.00 Z"
        fill="currentColor"
        fill-rule="evenodd"
      />
      <path
        class="hero-path hero-path-2"
      ...
      />
      <path
        class="hero-path hero-path-3 hero-spear"
      ...
      />
      <path
        class="hero-path hero-path-6 hero-spear"
