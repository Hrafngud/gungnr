...
import HostRuntimeSignalsSection from '@/components/host/HostRuntimeSignalsSection.vue'
...
const runtimeSignalsRef = ref<InstanceType<typeof HostRuntimeSignalsSection> | null>(null)
...
const hostDataRefreshing = computed(() =>
  containersLoading.value || usageLoading.value || localProjectsLoading.value,
)
...
const refreshHostData = async () => {
  ...
  runtimeSignalsRef.value?.loadRuntimeSnapshot()
}
...
<template>
  <section class="page space-y-10">
    ...
    <div class="grid gap-6">
      <HostRuntimeSignalsSection ref="runtimeSignalsRef" />
